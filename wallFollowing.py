...
while True:
    ...
    differenceRight = goalInchesRight - sens.getProxRightInches()
    newOutput =  -constantKp * difference #* difference
    newOutputRight = -constantKp * difference
    # print(abs(difference))
    # print(enc.getSpeeds())
    # if abs(difference) > StopMovingMarginOfError and moving:
    #     moving = True
    if differenceRight < -5: # based on reliability from sensor data plotting
        serv.setSpeedsVW(newOutput, turningRadius)
    else:
        serv.setSpeedsIPS(newOutput, newOutput)
        # serv.setSpeedsIPS(newOutput, newOutput)
    straightenPath(newOutputRight, newOutputRight)
